...
function danceAddStudentDetailInfo(opts) {
    ...
    function doAjaxStuDetail() {
        var cond = {'student_id': cur_stu_id, 'page': no, 'rows': 1};
        $.extend(cond, condition);
        console.log(url, '_details_get', cond);
        ...
        $.ajax({
            ...
            console.log('get stu detail',data);
            $.extend(true, oldStu, data);
            //console.log(oldStu);
            setStudentDetail(data);
            cur_stu_id = oldStu.rows.id;
        }).fail(function(jqXHR, textStatus, errorThrown) {
            var msg = "请求失败}。错误码：{0}({1})".format(jqXHR.status, errorThrown);
            $.messager.alert('提示', msg, 'info');
        });
    ...
    // 将 学员详细信息 设置 到页面控件中
    function setStudentDetail(data) {
    ...
    function onSave() {
        ...
        console.log('send',stuInfo,'url:', url);
        $.ajax({
            method: "POST",
            url: url,
...
function danceAddReceiptStudyDetailInfo(condition, uid, options) {
    ...
    function mmAddItems(mmId, data, mbId) {       // 菜单<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        for(var m = 0; m < change[0].length; m++){ // 删除
            var itemEl = $('#'+change[0][m]+'_'+ori_uid)[0];  // the menu item element
            ...
            for(var j = 0; j < data.length; j++){
                if(change[1][i] === data[j].class_no){
                    $(mmId).menu('appendItem', {
                        ...
                        iconCls: 'icon-ok',
                        id: data[j].class_no + '_'+ori_uid,     // bug fix: 菜单id重复，当 收费单（学费）详细信息 和 收费单（学费）详细信息[新增] 同时打开，并同时存在相同学员信息时，导致删错 报班信息。
...
function danceAddTabReceiptStatByMonth(title, tableId, condition) {
    ...
    if ($(parentDiv).tabs('exists', title)) {
        ...
        var content = '<div style="min-width:1024px;width:100%;height:100%"><div id=' + tableId + ' style="width: 100%;height:300px;"></div></div>';
        $(parentDiv).tabs('add', {
            title: title,
            content: content,
            closable: true
