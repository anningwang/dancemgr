 ...
 * @param value
 * @returns {{discount_rate_id: string, discount_rate: string}}
 */
function dcCalcDiscountRate(value) {
    var str = Number(value).toFixed(4);
    str = dcTrimStringZero(str);
    return {'discount_rate_id': str, 'discount_rate': str * 100 + '%' };
}
    ...
    $(dg).datagrid({
        ...
        toolbar: [{
            ...
            handler:function(){
                ...
                //console.log(cond);
                danceAddStudentDetailInfo('/static/html/_student.html',url,cond);
            }
        }, {
            ...
            handler:function(){
                ...
                if (row.length === 0) {
                    ...
                    var cond = $(dg).datagrid('options').queryParams;
                    //console.log(cond);
                    danceAddStudentDetailInfo('/static/html/_student.html', url, cond, row[0].id);
                }
            }
        }, {
...
function danceAddReceiptStudyDetailInfo( page, url, condition, uid) {
    ...
    var dcDiscRateOrig = [ {discount_rate_id: '1', discount_rate:'100%' },    // 折扣率
        {discount_rate_id: '0.95', discount_rate:'95%' }];
            ...
            onLoad : function (panel) {
                ...
                $('#'+dgReceiptComm).attr('id', dgReceiptComm+=uid).datagrid({  // 收费单（学费） ||||||||||||||||||||||
                    ...
                    onEndEdit: function (index, row){
                        if (index === 0) {
                            ...
                            dcSchoolId = parseInt($(ed.target).combobox('getValue'));
                        }
                    },
                    ...
                    onBeginEdit: function (index,row) {
                        ...
                        $(editors[3].target).combobox({     // 折扣率
                            onClick: function (record) {    // 折扣率 combobox 点击事件
                                var dg = $('#'+dgStudyFee);
                                var row = $(dg).datagrid("getSelected");
                                row.discount_rate_id =  record.discount_rate_id;
                                row.discount_rate = record.discount_rate;
                                var term = $(editors[1].target).textbox('getValue');
                                dgStudyFeeUpdateCellByTerm(edIndexStudyFee, term);
                            }
                        }).combobox('loadData', dcDiscRate).combobox('setValue', row['discount_rate_id']);
                    },
                    onEndEdit: function onEndEdit(index, row){
                        ...
                        var edDiscRate = $(this).datagrid('getEditor', { index: index, field: 'discount_rate' });
                        row.discount_rate = $(edDiscRate.target).combobox('getText');
                ...
                $('#'+dgTm).attr('id', dgTm+=uid).datagrid({       // 教材费   -----------------------------------------
                    ...
                    onBeginEdit: function (index,row) {
                        ...
                        $(edClass).combogrid({
                            data: filterClassBySchool(classlist, dcSchoolId)
                        ...
                        $(edTmName).combogrid({
                            ...
                            onSelect: function (index,record) {
                                ...
                                if (!row.tm_num) {
                                    row.tm_num = 1;
                                    setDgCellTextEx(dg, edIndexTm, 'tm_num', row.tm_num);
                                }
                                if (!row.is_got) {
                                    row.is_got = '是';
                ...
                dgParam[dgReceiptComm] = {idx: undefined, dg: '#'+dgReceiptComm, field: undefined};
    ...
    function doAjaxReceiptDetail() {
        ...
        $.ajax({
            ...
            dcSetArrearageStyle();
            //apiSetDgCellText('#'+dgReceiptComm, 1, 'student_id', data.row.student_id);
    ...
    function newReceipt() {
        ...
        setDgCellTextWithRowData(dg, 1, 'c2', '[关联学员姓名]');
     ...
     * @param value
     */
    function dgReceiptClickCell(index,field,value) {
        dgStudyFeeEndEditing();
        dgTmEndEditing();
        ...
        if (dgParam[dgReceiptComm].idx !== index) {
            ...
            if (index === 0){
                ...
                $(eds.target).combobox({
                    ...
                    data:filterSchool(schoollist, dcSchoolId)
                }).combobox('setValue', dcSchoolId);
            }else if (index === 1) {
    ...
    function onClickCellStudyFee(index, field) {
        ...
        if (edIndexStudyFee !== index) {
            ...
            // 优惠金额，修改优化金额后，更新 折扣率、应收金额、实收金额和学费欠费。
            $(editors[2].target).textbox('textbox').bind("input propertychange",function(){
                ...
                for (var i = 0; i < dcDiscRate.length; i++) {
                    if (dr.discount_rate_id === dcDiscRate[i].discount_rate_id) {
                        found = true;
                        break;
                    }
                }
                ...
                $(editors[3].target).combobox('loadData', dcDiscRate);
                row.discount_rate_id = dr.discount_rate_id;
                row.discount_rate = dr.discount_rat;
                $(editors[3].target).combobox('setValue', dr.discount_rate_id);
            });
            ...
            $(editors[4].target).textbox('textbox').bind("input propertychange",function(){
                ...
                var val;
                val = (val = apiGetDgCellText(dgRc, 3, 'c2')) ? val+diffReal : diffReal;
                setDgCellTextWithRowData(dgRc, 3, 'c2', val);   // 实收费合计
                val = (val = apiGetDgCellText(dgRc, 3, 'c4')) ? val+diffArrearage : diffArrearage;
                setDgCellTextWithRowData(dgRc, 3, 'c4', val);   // 学费欠费
    ...
    function dgTmUpdateCell(index, parms) {
        ...
        if (parms && (parms.num || parms.num === '')) {
            row.tm_num = parms.num;
        } else {
            var edNum = $(dg).datagrid('getEditor', {index: index, field: 'tm_num'});       // 教材 数量
            if (edNum) {
                row.tm_num = parseInt($(edNum.target).textbox('getValue'));
            }
        }

        if (row.tm_price_sell && row.tm_num){
            row.fee = dcTrimZero(row.tm_price_sell * row.tm_num);
    ...
    function dcCalcFeeByTm(difference) {
        var dg = $('#'+dgReceiptComm);
        var val;
        val = (val = apiGetDgCellText(dg, 2, 'c2')) ? val+difference : difference;
        setDgCellTextWithRowData(dg, 2, 'c2', val); // 教材费
        val = (val = apiGetDgCellText(dg, 2, 'c6')) ? val+difference : difference;
        setDgCellTextWithRowData(dg, 2, 'c6', val); // 费用合计
        val = (val = apiGetDgCellText(dg, 3, 'c2')) ? val+difference : difference;
        setDgCellTextWithRowData(dg, 3, 'c2', val); // 实收费合计
    }
    ...
    function dcCalcFeeByStudy(diffWanted, diffReal, diffArrearage) {
        ...
        setDgCellTextWithRowData(dg, 1, 'c6', val); // 应收学费
        val = (val = apiGetDgCellText(dg, 2, 'c6')) ? val+diffWanted : diffWanted;
        setDgCellTextWithRowData(dg, 2, 'c6', val); // 费用合计
        val = (val = apiGetDgCellText(dg, 3, 'c2')) ? val+diffReal : diffReal;
        setDgCellTextWithRowData(dg, 3, 'c2', val);   // 实收费合计
        val = (val = apiGetDgCellText(dg, 3, 'c4')) ? val+diffArrearage : diffArrearage;
        setDgCellTextWithRowData(dg, 3, 'c4', val);   // 学费欠费
    ...
    function setSchoolName(schoollist) {
        if (uid <=0 && schoollist.length) {
            ...
            dcSchoolId = schoollist[0].school_id;
        }
    }
     ...
     * @returns {*}
     */
    function filterClassBySchool(classList, school_id) {
        if (condition.school_id !== 'all') { // 已经锁定分校及班级，故不需要过滤班级
            return classList;
        }
     ...
     * @param school_id
     */
    function filterSchool(schoolList, school_id) {
        if (uid <= 0) {
            return schoolList;
        }
...
/*
    ...
    function packageReceipt() {
        ...
        recpt.row.school_id = dcSchoolId;
        recpt.row.school_name = rows[0].c4;
        recpt.row.deal_date = rows[0].c6;
        recpt.row.student_id = apiGetDgCellText(dgRecpt, 1, 'student_id');
    ...
    /**
     * 向 datagrid 添加 editor
     * @param dg
     * @param index
     * @param field
     * @returns {*}
     */
    function dcAddEditor(dg, index, field) {
        if (index >= 0 && index < editors.length) {
            if (editors[index].hasOwnProperty(field)) {
                $(dg).datagrid('addEditor', {field:field, editor:editors[index][field]});
                return editors[index][field];
            }
        }
    }
